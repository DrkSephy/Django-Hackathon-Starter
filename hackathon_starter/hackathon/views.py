...
# Scripts
from hackathon.scripts.steam import gamespulling, steamidpulling
from hackathon.scripts.github import *
from hackathon.scripts.tumblr import TumblrOauthClient

# currently an
from hackathon.scripts.twilioapi import *
from hackathon.scripts.instagram import *
from hackathon.scripts.scraper import steamDiscounts
from hackathon.scripts.quandl import *
from hackathon.scripts.twitter import TwitterOauthClient
from hackathon.scripts.nytimes import *
from hackathon.scripts.meetup import *
from hackathon.scripts.linkedin import LinkedinOauthClient
from hackathon.scripts.yelp import requestData
from hackathon.scripts.facebook import *
from hackathon.scripts.googlePlus import *
from hackathon.scripts.dropbox import *
from hackathon.scripts.foursquare import *
    ...
    if not request.user.is_active:
        if request.GET.items():
            if profile_track == 'github':
                ...
                try:
                    user = user_model.objects.get(username=getGithub.username + '_github')
                except user_model.DoesNotExist:
                    username = getGithub.username + '_github'
                    new_user = user_model.objects.create_user(username, username + '@madewithgithub.com', 'password')
                    new_user.save()
                    try:
                ...
                try:
                    user = user_model.objects.get(
                        username=getTwitter.username + '_twitter')  # (username=getTwitter.username)
                except user_model.DoesNotExist:
                    username = getTwitter.username + '_twitter'
                    new_user = user_model.objects.create_user(username, username + '@madewithtwitter.com', 'password')
                    new_user.save()
                ...
                try:
                    user = user_model.objects.get(username=getInstagram.user_data['username'] + '_instagram')
                except user_model.DoesNotExist:
                    username = getInstagram.user_data['username'] + '_instagram'
                    new_user = user_model.objects.create_user(username, username + '@madewithinstagram.com', 'password')
                    new_user.save()
                ...
                try:
                    user = user_model.objects.get(username=getLinkedIn.user_id + '_linkedin')
                except user_model.DoesNotExist:
                    username = getLinkedIn.user_id + '_linkedin'
                    new_user = user_model.objects.create_user(username, username + '@madwithlinkedin.com', 'password')
                    new_user.save()
                    try:
                ...
                try:
                    user = user_model.objects.get(username=username + '_facebook')
                except user_model.DoesNotExist:
                    new_user = user_model.objects.create_user(username + '_facebook', username + '@madewithfacbook',
                                                              'password')
                    new_user.save()
                ...
                if not getTumblr.is_authorized:
                    ...
                    try:
                        user = user_model.objects.get(username=getTumblr.username + '_tumblr')
                    except user_model.DoesNotExist:
                        username = getTumblr.username + '_tumblr'
                        new_user = user_model.objects.create_user(username, username + '@madewithtumblr.com',
                                                                  'password')
                        new_user.save()
                        try:
                ...
                try:
                    user = user_model.objects.get(username=username + '_google')
                except user_model.DoesNotExist:
                    new_user = user_model.objects.create_user(username + '_google', username + '@madewithgoogleplus',
                                                              'password')
                    new_user.save()
                    ...
                    except GoogleProfile.DoesNotExist as e:
                ...
                try:
                    user = user_model.objects.get(username=username + '_dropbox')
                except user_model.DoesNotExist:
                    new_user = user_model.objects.create_user(username + '_dropbox', username + '@madewithdropbox',
                                                              'password')
                    new_user.save()
                    ...
                    except DropboxProfile.DoesNotExist as e:
                ...
                try:
                    user = user_model.objects.get(username=username + '_foursquare')
                except user_model.DoesNotExist:
                    new_user = user_model.objects.create_user(username + '_foursquare',
                                                              username + '@madewithfoursquare',
                                                              'password')
                    new_user.save()
                    ...
                    except FoursquareProfile.DoesNotExist as e:
        ...
        if request.GET.items():
            ...
            if profile_track == 'github':
                ...
                try:
                    ...
                    username = githubUser.username + '_tumblr'
                    new_user = user_model.objects.create_user(username, username + '@madewithtumblr.com', 'password')
                    new_user.save()
...
def meetup(request):
    REDIRECT_URI = '/hackathon/meetupToken'
...
def meetupToken(request):
    ...
    return HttpResponseRedirect('/hackathon/meetupUser/')


def meetupUser(request):
    if not MeetupToken.objects.all().exists():
        return HttpResponseRedirect('/hackathon/meetup')
...
def instagramMediaByLocation(request):
    ...
    if request.method == 'GET':
        if request.GET.items():
            ...
            if request.user in user_model.objects.all():
